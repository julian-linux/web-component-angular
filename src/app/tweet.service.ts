...
export default class TweetService {
  ...
  getTweets(text: string, dir: string): any {
    // this.http.get('https://api.twitter.com/1.1/search/tweets.json?q=twitterdev%20new%20premium', {})
    //   .subscribe(data => {
    ...
    return response.statuses
      .map(tweet => this.parseResponse(tweet))
      .filter(tweet => tweet.text[0].includes(text))
      .sort(() => dir === 'asc' ? -1 : 1);
  }
  ...
  parseResponse(data) {
    return {
      id: data.id,
      created: data.created_at.split(" "),
      text: data.text.split("…").map(text => text.trim()),
